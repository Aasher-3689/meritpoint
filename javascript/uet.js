...
function updateResult() {
    ...
    const nonEcatResultDiv = document.getElementById("non-ecat-result");
    let result = 0;
    let nonEcatResult = 0;
    ...
    if (!isNaN(matricT) && !isNaN(matricO) && matricT!=0 && matricT>=matricO) {
        ...
        nonEcatResult += (matricO/matricT) * 25;
        resultDiv.innerText = result.toFixed(2);
        nonEcatResultDiv.innerText = `Non-Ecat Agg: ${nonEcatResult.toFixed(2)}%`;
        updateRing(result);
    }

    if (!isNaN(fscT) && !isNaN(fscO) && fscT!=0 && fscT>=fscO) {
        result += (fscO/fscT) * 50;
        nonEcatResult += (fscO/fscT) * 75;
        resultDiv.innerText = result.toFixed(2);
        nonEcatResultDiv.innerText = `Non-Ecat Agg: ${nonEcatResult.toFixed(2)}%`;
        updateRing(result);
    }
    ...
    if ((isNaN(matricO) || isNaN(matricT) || matricT<matricO) && (isNaN(fscT) 
        || isNaN(fscO) || fscT<fscO) && (isNaN(testT) || isNaN(testO) || testT<testO)) {
            ...
            nonEcatResultDiv.innerText = `Non-Ecat Agg: 00.00%`;
            updateRing(0);
    }
}
